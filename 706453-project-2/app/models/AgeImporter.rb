...
class Ageimporter
    ...
    # have not specified them here. But If we choose another source 
             ...
             feed.items.each do |item|
                tags= tag_article(item.title.to_s.gsub(/,/,''))
             	article = Article.new(title: item.title.to_s, 
                                      summary: (item.description).match(/\<\/p\>[\w ?,''""]+/).to_s.gsub(/\<\/p\>/,'').gsub(/\"/,''), 
                                      source: "The Age", 
                                      image: (item.description).match(/http.+\.(jpg|png)/), 
                                      link: item.link, 
